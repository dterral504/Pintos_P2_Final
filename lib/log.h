...
static inline const char *log_level_name(LoggingLevel level)
{
    switch (level)
    {
    case L_TRACE:
...
static inline const char *log_level_color(LoggingLevel level)
{
    switch (level)
    {
    case L_TRACE:
...
#define log(LEVEL, fmt...)                                                                                                                                               \
    ...
    {                                                                                                                                                                    \
        ...
        {                                                                                                                                                                \
            snprintf((char *)&log_before_buf, LOG_MAX_BEFORE_SIZE, "%s%-5s | %s:%d:%s() ", log_level_color(level), log_level_name(level), __FILE__, __LINE__, __func__); \
